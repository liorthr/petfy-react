...
const Card = () => {
    ...
    const handleButtonClick = async (dogId) => {
        try {
            const response = await fetch(`http://localhost:3000/dogs/dogId/${dogId}`);
            if (!response.ok) {
                throw new Error('Failed to fetch dog data');
            }
            const data = await response.json();
            console.log('Dog data:', data);
            // Do something with the fetched dog data
        } catch (error) {
                    ...
                    {DataDogs.map((dog, index) => (
                        <div key={index} className="card-type" style={{ backgroundImage: dog.dog_picture && dog.dog_picture[0] ? `url(${dog.dog_picture[0]})` : '' }}>
                            ...
                            <div className="bottom-card">
                                ...
                                {/* Pass the dog id to the handleButtonClick function */}
                                <button className='card_button' onClick={() => handleButtonClick(dog.dog_id)}>Dater ce chien</button>
                            </div>
                        </div>
                    ))}
                </div>
